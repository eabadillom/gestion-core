package com.ferbo.gestion.core.model.catalogo.egresos;
...
@NamedQueries({
        @NamedQuery(name = "StatusEgreso.findByClave", query = "SELECT s FROM StatusEgreso s WHERE s.clave = :clave"),
        @NamedQuery(name = "StatusEgreso.findActivosOInactivos", query = "SELECT s FROM StatusEgreso s WHERE s.activo = :activo")
})
@Entity
@Table
public class StatusEgreso implements Serializable {
    ...
    @Column(name = "tx_status_egre", length = 255)
    ...
    public String toString() {
        return "StatusEgreso [id=" + id + ", clave=" + clave + ", nombre=" + nombre + ", descripcion=" + descripcion
